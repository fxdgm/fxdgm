...
for i, phi_L in enumerate(phi_left_vector):
    axs[0].plot(x[i], phi[i], lw=lw, label=r'$\delta \varphi$ = ' + str(phi_L))
# axs[0].legend()
axs[0].set_xlabel('$x$ [-]', fontsize=labelsize)
axs[0].set_ylabel(r'$\varphi$ [-]', fontsize=labelsize)
...
axs[1].set_ylabel(r'$\varphi/\delta \varphi$ [-]', fontsize=labelsize)
