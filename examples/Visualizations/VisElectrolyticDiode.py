...
for bias, (y_A, y_C, y_S, phi, p, x, y) in enumerate(zip(y_A_packed, y_C_packed, y_S_packed, phi_packed, p_packed, x_packed, y_packed)): 
    # Potential
    c = axs[bias,0].tricontourf(x, y, phi, cmap=color_theme_potential, levels=vmap_potential)
    ...
    axs[bias,0].set_title(rf'$\varphi \in ({round(np.min(phi),2)},{round(np.max(phi), 2)})$', fontsize=titlesize)

    # Concentration A
    c = axs[bias,1].tricontourf(x, y, y_A, cmap=color_theme_concentration, levels=vmap_concentrations)
    axs[bias,1].tricontour(x, y, y_A, colors='black', levels=levels_contour)
    axs[bias,1].tick_params(axis='both', labelsize=labelsize)
    axs[bias,1].set_title(rf'$y_A \in ({round(np.min(y_A),2)},{round(np.max(y_A), 2)})$', fontsize=titlesize)
    ...
    axs[bias,2].set_title(rf'$y_C \in ({round(np.min(y_C),2)},{round(np.max(y_C), 2)})$', fontsize=titlesize)
    ...
    axs[bias,3].set_title(rf'$y_S \in ({round(np.min(y_S),2)},{round(np.max(y_S), 2)})$', fontsize=titlesize)
    ...
    axs[bias,4].set_title(rf'$p \in ({int(round(np.min(p),0))},{int(round(np.max(p), 0))})$', fontsize=titlesize)
