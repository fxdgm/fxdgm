...
# BCS according to Fuhrmann - FuelCells
nRef = 55.4 # [mol/L]
Molarity = 0.5
...
phi_L_dim = 0.6 # [V] ->  500 mV # ! Change back to 0.5 for Fuhrmann Fuel-Cells, 0.6 for Dereyer - Bulk-Surface
phi_R_dim = 0.0 # [V]
...
a2 = (p_ref)/(nR_m * k * T)
Lambda2, a2 # values not sure, as T, chi and L_ref not given in Fuhrmann paper

# Solver parameters
number_cells = 1028*16#1024*4#128
relax_param = 0.005#0.001##0.01
refinement_style = 'log'
max_iter = 100_000
return_type = 'Vector'
rtol = 1e-8 # ! Change back to 1e-8 
...
y_A_, y_C_, phi_, p_, x_ = solve_System_4eq(phi_L_dimless, phi_R_dimless, pR, z_A, z_C, y_A_R, y_C_R, K, Lambda2, a2, number_cells=1024, solvation=0, PoissonBoltzmann=False, relax_param=relax_param, refinement_style=refinement_style, return_type=return_type, max_iter=max_iter, rtol=rtol)
y_A_dimless.append(y_A_)
y_C_dimless.append(y_C_)
y_S_dimless.append(1 - y_A_ - y_C_)
phi_dimless.append(phi_)
...
y_A_, y_C_, phi_, p_, x_ = solve_System_4eq(phi_L_dimless, phi_R_dimless, pR, z_A, z_C, y_A_R, y_C_R, K, Lambda2, a2, number_cells=1024, solvation=8, PoissonBoltzmann=False, relax_param=relax_param, refinement_style=refinement_style, return_type=return_type, max_iter=max_iter, rtol=rtol)
y_A_dimless.append(y_A_)
y_C_dimless.append(y_C_)
y_S_dimless.append(1 - y_A_ - y_C_)
phi_dimless.append(phi_)
