...
for K in K_vec:
    ...
    for i, phi_bcs in enumerate(Phi_Pot_Diff_dimless):
        y_A_, y_C_, phi_, p_, x_ = solve_System_4eq(phi_bcs, phi_R, p_right, z_A, z_C, y_A_R, y_C_R, K, Lambda2, a2, number_cells, solvation=kappa, refinement_style='hard_log', rtol=rtol, max_iter=max_iter, return_type='Vector', relax_param=relax_param)
...
for i, q_dl in enumerate(Q_DL_dimless_):
    plt.plot(Phi_Pot_Diff_dimless, q_dl, color=colors[i], label=f'K: {K_vec[i]}')
plt.grid()
...
# Save the results
np.savez('../../Data/DoubleLayerCapacity/DLKap_Compressibility_numerically.npz', Lambda2=Lambda2, a2=a2, K_vec=K_vec, kappa=kappa, z_A=z_A, z_C=z_C, phi_R=phi_R, p_R=p_right, Volt_start=Volt_start, Volt_end=Volt_end, n_Volts=n_Volts, Phi_Pot_Diff_dim=Phi_Pot_Diff_dim, Phi_Pot_Diff_dimless=Phi_Pot_Diff_dimless, C_DL_dim=C_DL_dim_, C_DL_dimless=C_DL_dimless_, Q_DL_dim_=Q_DL_dim_, Q_DL_dimless_=Q_DL_dimless_, Molarity=Molarity)
