...
# Lambda2
Lambda2_vec = np.array([1e-4, 1e-5, 1e-6])
...
Q_DL_dimless_ = []
Q_DL_dim_ = []
for i in range(len(Lambda2_vec)):
    Q_inner_dimless_ = []
    Q_inner_dim_ = []
...
for i, q_dl in enumerate(Q_DL_dimless_):
    plt.plot(Phi_Pot_Diff_dimless, q_dl, color=colors[i], label=f'$\lambda^2$: {Lambda2_vec[i]}')
plt.grid()
